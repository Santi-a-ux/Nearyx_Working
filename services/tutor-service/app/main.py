    ...
    async with engine.begin() as conn:
                ...
                await conn.execute(text('''
                        CREATE TABLE IF NOT EXISTS "tutors".ratings (
                            ...
                            created_at TIMESTAMPTZ DEFAULT NOW(),
                            updated_at TIMESTAMPTZ DEFAULT NOW(),
                            CONSTRAINT uq_tutor_ratings_tutor_rater UNIQUE (tutor_user_id, rater_user_id)
                        )
                '''))
                await conn.execute(text('CREATE INDEX IF NOT EXISTS ix_tutors_ratings_tutor_user_id ON "tutors".ratings(tutor_user_id)'))
                await conn.execute(text('CREATE INDEX IF NOT EXISTS ix_tutors_ratings_rater_user_id ON "tutors".ratings(rater_user_id)'))
