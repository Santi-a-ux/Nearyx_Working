...
function MessagesPageContent() {
    ...
    try {
      ...
      const enriched = await Promise.all(
        list.map(async (conversation) => {
          const partnerId = conversation.other_participant_id || conversation.participant_ids.find((id) => id !== meId) || "";
          const profile = partnerId ? await fetchApi<UserProfileSummary>(`/users/profiles/${partnerId}`).catch(() => null) : null;
          return { ...conversation, partnerName: profile?.display_name || `Chat ${partnerId.slice(0, 8)}`, partnerAvatar: profile?.avatar_url };
        })
      );
  ...
  const loadConversationMessages = async (convId: string) => {
    try {
      ...
      setMessages(history.map((msg: any) => ({
        id: String(msg.id), conversation_id: convId, sender_id: String(msg.sender_id),
        receiver_id: receiverIdRef.current, content: msg.content, timestamp: msg.created_at, is_read: msg.is_read,
      })));
  ...
  useEffect(() => {
    fetch("/api/session")
      .then((r) => r.json())
      .then((user: any) => { setMeId(user.user_id); setMeRole(user.role ?? null); })
      .finally(() => setIsLoading(false));
  }, []);

  useEffect(() => { if (meId) loadConversations(); }, [meId]);

  useEffect(() => {
    if (!meId) return;
    if (receiverId && isUuid(receiverId)) void ensureConversation(receiverId);
  }, [meId, receiverId]);

  useEffect(() => { scrollToBottom(); }, [messages, conversationId]);
  ...
  useEffect(() => {
    ...
    const connectWebSocket = async () => {
      try {
        ...
        if (!tokenResponse.ok) throw new Error("Failed to fetch WS token");
        ...
        ws.onmessage = (event) => {
          try {
            ...
            setMessages((prev) => [...prev, {
              id: crypto.randomUUID(), conversation_id: data.conversation_id || conversationIdRef.current || undefined,
              sender_id: String(data.sender_id || ""), receiver_id: receiverIdRef.current,
              content: data.content || "", timestamp: new Date().toISOString(), is_read: false,
            }]);
          } catch (error) { console.error("Error parseando mensaje WS:", error); }
        };
        ws.onclose = () => setIsConnected(false);
        ws.onerror = () => setIsConnected(false);
        wsRef.current = ws;
        setSocket(ws);
      } catch (error) { console.error("Error conectando WebSocket:", error); setIsConnected(false); }
    };
  ...
  const activeConversation = useMemo<ConversationItem | null>(
    () => conversations.find((c) => c.other_participant_id === receiverId) || null,
    [conversations, receiverId]
  );

  const resolvedName = receiverProfile?.display_name || activeConversation?.partnerName || (receiverId ? `Chat ${receiverId.slice(0, 8)}` : "Selecciona un chat");
  const resolvedAvatar = receiverProfile?.avatar_url || activeConversation?.partnerAvatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=${receiverId || "empty"}`;
  ...
  const handleSendMessage = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!activeSocket || activeSocket.readyState !== WebSocket.OPEN || !meId || !conversationIdRef.current) {
      toast.error("No estás conectado al servidor de chat.");
      return;
    }
    activeSocket.send(JSON.stringify({ conversation_id: conversationIdRef.current, receiver_id: receiverIdRef.current, content: newMessage }));
    setMessages((prev) => [...prev, {
      id: Math.random().toString(), conversation_id: conversationIdRef.current!, sender_id: meId,
      receiver_id: receiverIdRef.current, content: newMessage, timestamp: new Date().toISOString(), is_read: false,
    }]);
    setNewMessage("");
      ...
      <div className="flex h-full min-h-0 w-full gap-3 p-3 lg:p-4">
        <aside className="flex min-h-0 w-[360px] flex-col overflow-hidden rounded-[22px] border border-[rgba(253,251,212,0.12)] bg-[var(--color-surface)] shadow-[0_24px_60px_rgba(56,36,13,0.18)]">
          <div className="flex-shrink-0 border-b border-[rgba(253,251,212,0.12)] px-5 py-4">
            <div className="flex items-center justify-between gap-4">
              <div>
                <p className="text-[11px] uppercase tracking-[0.26em] text-[var(--color-text-muted)]">Mensajes</p>
                <h2 className="mt-2 text-[1.05rem] font-semibold text-[var(--color-bg)]">Conversaciones</h2>
              </div>
          ...
          <div className="flex-shrink-0 border-b border-[rgba(253,251,212,0.12)] px-4 py-3">
            <div className="relative">
              ...
              <Input placeholder="Buscar conversaciones..." className="h-11 rounded-xl border border-[rgba(253,251,212,0.12)] bg-[rgba(253,251,212,0.06)] pl-10 text-[var(--color-bg)] placeholder:text-[rgba(253,251,212,0.5)] focus-visible:ring-0" />
            </div>
          </div>
              ...
              <div className="space-y-2 p-2.5">
                {conversations.length === 0 ? (
                  ...
                  conversations.map((c) => {
                    ...
                    return (
                      <Button type="button" key={c.id} onClick={() => openConversation(partnerId)} variant="ghost"
                        className={`group h-auto w-full rounded-[18px] border px-4 py-3 text-left transition-all duration-200 ${isActive ? "border-[rgba(253,251,212,0.16)] bg-[rgba(253,251,212,0.08)]" : "border-transparent hover:border-[rgba(253,251,212,0.12)] hover:bg-[rgba(253,251,212,0.05)]"}`}>
                        <div className="flex items-start gap-3">
                          <Avatar className="h-11 w-11 flex-shrink-0 border border-[rgba(253,251,212,0.14)]">
                            <AvatarImage src={c.partnerAvatar} />
                            <AvatarFallback className="bg-[var(--color-bg)] text-[var(--color-surface)]">{c.partnerName.slice(0, 2).toUpperCase()}</AvatarFallback>
                          </Avatar>
                          <div className="min-w-0 flex-1">
                            <div className="flex items-start justify-between gap-3">
                              <div className="min-w-0">
                                <div className="truncate text-sm font-semibold text-[var(--color-bg)]">{c.partnerName}</div>
                                <div className="mt-1 truncate text-xs text-[rgba(253,251,212,0.68)]">{c.last_message || "Sin mensajes"}</div>
                              </div>
                              {c.last_message_at && (
        ...
        <section className="flex min-w-0 flex-1 flex-col overflow-hidden rounded-[22px] border border-[rgba(253,251,212,0.12)] bg-[var(--color-surface)] shadow-[0_24px_60px_rgba(56,36,13,0.18)]">
            ...
            <div className="flex items-center justify-between gap-4">
              <div className="flex min-w-0 items-center gap-3">
                <Avatar className="h-11 w-11 flex-shrink-0 border border-[rgba(253,251,212,0.14)]">
                  ...
                  <AvatarFallback className="bg-[var(--color-bg)] text-[var(--color-surface)]">{resolvedName.slice(0, 2).toUpperCase()}</AvatarFallback>
                </Avatar>
                <div className="min-w-0">
                  <h3 className="truncate text-sm font-semibold text-[var(--color-bg)]">{resolvedName}</h3>
          ...
          {showBookingModal && receiverId && (
            <BookingModal studentId={receiverId} studentName={resolvedName} onClose={() => setShowBookingModal(false)} onSuccess={() => {}} />
          )}
                      ...
                      <div key={m.id} className={`flex items-end gap-3 ${isMe ? "flex-row-reverse" : ""}`}>
                        {!isMe && (
                          <Avatar className="h-8 w-8 flex-shrink-0 border border-[rgba(253,251,212,0.12)]">
                            <AvatarImage src={resolvedAvatar} />
                            <AvatarFallback className="bg-[var(--color-bg)] text-[var(--color-surface)] text-xs">{resolvedName.slice(0, 1).toUpperCase()}</AvatarFallback>
                          </Avatar>
                        )}
                        <div className={`flex max-w-[min(34rem,78%)] flex-col ${isMe ? "items-end" : "items-start"}`}>
                          <div className={`rounded-[18px] border px-4 py-3 shadow-[0_12px_24px_rgba(0,0,0,0.08)] ${isMe ? "rounded-br-md border-[rgba(253,251,212,0.16)] bg-[rgba(253,251,212,0.08)] text-[var(--color-bg)]" : "rounded-bl-md border-[rgba(253,251,212,0.14)] bg-[rgba(253,251,212,0.04)] text-[rgba(253,251,212,0.96)]"}`}>
                            <p className="break-words text-sm leading-6">{m.content}</p>
                          </div>
                          <span className="mt-1.5 text-[11px] text-[rgba(253,251,212,0.52)]">{time}</span>
                        </div>
                        {isMe && (
                          <Avatar className="h-8 w-8 flex-shrink-0 border border-[rgba(253,251,212,0.12)]">
                            <AvatarImage src={resolvedAvatar} />
          ...
          <div className="flex-shrink-0 border-t border-[rgba(253,251,212,0.12)] px-4 py-4 lg:px-5">
            <form className="flex items-end gap-3" onSubmit={handleSendMessage}>
              <Input value={newMessage} onChange={(e: ChangeEvent<HTMLInputElement>) => setNewMessage(e.target.value)}
              ...
              <Button type="submit" className="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-xl bg-[var(--color-bg)] text-[var(--color-surface)] transition-all hover:opacity-90">
